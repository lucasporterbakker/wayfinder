...
function handleHashNavigation() {
  ...
  if (hash) {
    ...
    if (targetElement) {
      // Advanced settings expansion is handled automatically by CSS now
...
function setupEventHandlers() {
  ...
  // Advanced settings toggle (optional element)
  ...
  // Note: advancedToggle is optional and may not exist in current UI design
  ...
  document
    ...
    ?.addEventListener('change', saveDebugMode);

  document
    .getElementById('verificationStrategyToggle')
    ?.addEventListener('change', (e) => {
      // Strategy toggle just controls visibility - no storage needed
    });
  ...
  document
    .getElementById('verificationGatewayModeToggle')
    ?.addEventListener('change', (e) => {
      // Gateway mode toggle just controls visibility - no storage needed
    });

  // Verification gateway mode selection
  document
...
function setupExpandableSections() {
  ...
  // Setup advanced options expansion
  document.querySelectorAll('.advanced-option .advanced-header').forEach((header) => {
    header.addEventListener('click', (e) => {
      // Don't prevent checkbox clicks, but handle header clicks
      if (e.target.tagName === 'INPUT') {
        return; // Let the checkbox handle its own state
      }
      
      // If clicking on the label or anywhere else in the header, toggle the checkbox
      const checkbox = header.querySelector('input[type="checkbox"]');
      if (checkbox) {
        checkbox.checked = !checkbox.checked;
        // Trigger change event for any additional handling
...
function toggleAdvancedSettings() {
  // Advanced settings are now always visible in the current UI design
  console.log('[DEBUG] toggleAdvancedSettings called - advanced settings are always expanded in current design');
}
...
function setupRoutingStrategyDetails() {
  ...
  // Setup click handlers for standalone verification strategy section
  document
    .querySelectorAll('.verification-strategy-config .strategy-option')
    .forEach((option) => {
      const radio = option.querySelector('input[type="radio"]');
      const header = option.querySelector('.strategy-header');
...
async function loadCurrentSettings() {
  try {
    ...
    const enableVerificationCache = settings.enableVerificationCache !== false; // Default true
    const verificationCacheToggle = document.getElementById('enableVerificationCache');
    if (verificationCacheToggle) {
      verificationCacheToggle.checked = enableVerificationCache;
    }

    // Set default states for advanced option expansion toggles
    const verificationStrategyToggle = document.getElementById('verificationStrategyToggle');
    if (verificationStrategyToggle) {
      verificationStrategyToggle.checked = true; // Default expanded
    }

    const verificationGatewayModeToggle = document.getElementById('verificationGatewayModeToggle');
    if (verificationGatewayModeToggle) {
      verificationGatewayModeToggle.checked = true; // Default expanded
    }
    
    // Advanced settings are now always expanded in current UI design
    ...
    // Load verification gateway settings
    const verificationGatewayMode = settings.verificationGatewayMode || 'automatic';
    const gatewayModeRadio = document.querySelector(
      `input[name="verificationGatewayMode"][value="${verificationGatewayMode}"]`,
    );
    if (gatewayModeRadio) {
      gatewayModeRadio.checked = true;
    }

    const verificationGatewayCount = settings.verificationGatewayCount || 3;
    const gatewayCountSlider = document.getElementById('verificationGatewayCount');
    const gatewayCountValue = document.getElementById('gatewayCountValue');
    if (gatewayCountSlider) {
      gatewayCountSlider.value = verificationGatewayCount;
    }
    if (gatewayCountValue) {
      gatewayCountValue.textContent = verificationGatewayCount;
    }
    ...
    updateTTLDisplay(gatewayCacheTTL);

    // Load network configuration
    const processId = settings.processId || '';
    const processIdEl = document.getElementById('processId');
    if (processIdEl) {
      processIdEl.value = processId;
    }

    const aoCuUrl = settings.aoCuUrl || '';
    const aoCuUrlEl = document.getElementById('aoCuUrl');
    if (aoCuUrlEl) {
      aoCuUrlEl.value = aoCuUrl;
    }

    // Setup trusted gateway settings
    await setupTrustedGatewaySettings();

    // Enable test buttons for network configuration
    enableTestButtons();
  } catch (error) {
...
async function handleRoutingStrategyChange(event) {
  ...
  const staticDetails = document.querySelector('.strategy-option[data-strategy="static"] .strategy-details');
  
  console.log(`[DEBUG] Strategy changed to: ${strategy}`);
  console.log(`[DEBUG] staticConfig element:`, staticConfig);
  console.log(`[DEBUG] staticDetails element:`, staticDetails);
  
  if (staticDetails) {
    if (strategy === 'static') {
      staticDetails.style.display = 'block';
      console.log(`[DEBUG] Showing static details`);
      // Populate the gateway dropdown when static is selected
      await populateGatewayDropdown();
    } else {
      staticDetails.style.display = 'none';
      console.log(`[DEBUG] Hiding static details`);
    }
  }
  
  // Legacy support for staticConfig
  if (staticConfig) {
    if (strategy === 'static') {
...
async function saveDebugMode(event) {
...
async function populateGatewaySelectionList(selectedGateways) {
  ...
  gateways.forEach((gateway) => {
    ...
    checkboxDiv.className = 'gateway-selection-item';
    ...
    label.innerHTML = `${gateway.fqdn} • ${formatStake(totalStake)}`;
...
async function updateTrustedGatewaysPreview() {
  ...
  if (trustedGateways.length === 0) {
    previewEl.innerHTML =
      '<div class="trusted-gateway-item"><div class="trusted-gateway-info"><div class="gateway-url">No gateways selected</div></div></div>';
    return;
  }
  ...
  trustedGateways.forEach((gateway) => {
    ...
    item.className = 'trusted-gateway-item';
    item.innerHTML = `
      <div class="trusted-gateway-info">
        <div class="gateway-url">${gateway.fqdn}</div>
        <div class="gateway-stake">${formatStake(totalStake)}</div>
      </div>
    `;
...
// TTL change timeout for debouncing
let ttlChangeTimeout = null;

async function handleGatewayCacheTTLChange(event) {
  const ttl = parseInt(event.target.value);
  
  // Update the display value immediately
  updateTTLDisplay(ttl);
  
  // Save to storage immediately but debounce the toast and wayfinder reset
  await chrome.storage.local.set({ gatewayCacheTTL: ttl });
  
  // Clear existing timeout
  if (ttlChangeTimeout) {
    clearTimeout(ttlChangeTimeout);
  }
  
  // Set new timeout to show toast and reset wayfinder after user stops dragging
  ttlChangeTimeout = setTimeout(async () => {
    try {
      await chrome.runtime.sendMessage({ message: 'resetWayfinder' });
      showToast('Gateway cache TTL updated', 'success');
    } catch (error) {
...
async function clearCache() {
  if (confirm('Clear extension cache? This will remove cached gateway data and performance metrics.')) {
    try {
      await chrome.storage.local.remove([
        'gatewayPerformance',
        'gatewayUsageHistory',
      ...
      // Also clear verification cache via background script
      try {
        await chrome.runtime.sendMessage({ message: 'clearVerificationCache' });
      } catch (error) {
        console.log('Background script cache clear not available:', error);
      }
...
async function resetExtension() {
  if (confirm('Reset all extension data? This will restore all settings to defaults and clear all data. This cannot be undone.')) {
    try {
      // Clear all extension storage
      await chrome.storage.local.clear();
      
      showToast('Extension reset successfully. Please reload the page.', 'success');
...
// Helper function to format TTL values for display
function updateTTLDisplay(ttlSeconds) {
  const ttlValueEl = document.getElementById('ttlValue');
  if (!ttlValueEl) return;

  if (ttlSeconds < 60) {
    ttlValueEl.textContent = `${ttlSeconds} seconds`;
  } else if (ttlSeconds < 3600) {
    const minutes = Math.floor(ttlSeconds / 60);
    ttlValueEl.textContent = `${minutes} minute${minutes !== 1 ? 's' : ''}`;
  } else if (ttlSeconds < 86400) {
    const hours = Math.floor(ttlSeconds / 3600);
    ttlValueEl.textContent = `${hours} hour${hours !== 1 ? 's' : ''}`;
  } else {
    const days = Math.floor(ttlSeconds / 86400);
    ttlValueEl.textContent = `${days} day${days !== 1 ? 's' : ''}`;
  }
}

// Test Process ID function
async function testProcessId() {
  const processIdInput = document.getElementById('processId');
  const testButton = document.getElementById('testProcessId');
  
  if (!processIdInput || !testButton) return;
  
  const processId = processIdInput.value.trim();
  if (!processId) {
    showToast('Please enter a Process ID to test', 'warning');
    return;
  }
  ...
  try {
    // Test the process ID by making a request to the AR.IO network
    const response = await fetch(`https://ar-io.net/process/${processId}`, {
      method: 'GET',
      signal: AbortSignal.timeout(10000),
    });

    if (response.ok) {
      showToast('Process ID is valid and reachable!', 'success');
    } else {
      showToast(`Process ID test failed with status ${response.status}`, 'warning');
    }
  } catch (error) {
    showToast('Process ID test failed - unable to reach AR.IO network', 'error');
  } finally {
    testButton.disabled = false;
    testButton.textContent = 'Test';
  }
...
// Test AO Compute Unit URL function
async function testAoCuUrl() {
  const aoCuUrlInput = document.getElementById('aoCuUrl');
  const testButton = document.getElementById('testAoCuUrl');
  
  if (!aoCuUrlInput || !testButton) return;
  
  const aoCuUrl = aoCuUrlInput.value.trim();
  if (!aoCuUrl) {
    showToast('Please enter an AO Compute Unit URL to test', 'warning');
    return;
  }
  ...
  try {
    // Test the AO CU URL by making a health check request
    const response = await fetch(`${aoCuUrl}/health`, {
      method: 'GET',
      signal: AbortSignal.timeout(10000),
    });

    if (response.ok) {
      showToast('AO Compute Unit URL is reachable!', 'success');
    } else {
      showToast(`AO CU test failed with status ${response.status}`, 'warning');
    }
  } catch (error) {
    showToast('AO Compute Unit URL test failed - unable to reach endpoint', 'error');
  } finally {
    testButton.disabled = false;
    testButton.textContent = 'Test';
  }
...
function enableTestButtons() {
  const processIdInput = document.getElementById('processId');
  const testProcessIdButton = document.getElementById('testProcessId');
  const aoCuUrlInput = document.getElementById('aoCuUrl');
  const testAoCuUrlButton = document.getElementById('testAoCuUrl');

  if (processIdInput && testProcessIdButton) {
    testProcessIdButton.disabled = !processIdInput.value.trim();
    processIdInput.addEventListener('input', () => {
      testProcessIdButton.disabled = !processIdInput.value.trim();
    });
    testProcessIdButton.addEventListener('click', testProcessId);
  }

  if (aoCuUrlInput && testAoCuUrlButton) {
    testAoCuUrlButton.disabled = !aoCuUrlInput.value.trim();
    aoCuUrlInput.addEventListener('input', () => {
      testAoCuUrlButton.disabled = !aoCuUrlInput.value.trim();
    });
    testAoCuUrlButton.addEventListener('click', testAoCuUrl);
  }
}
