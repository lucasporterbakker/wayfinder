...
function createGatewayCard(gateway) {
  ...
  const uptime = performance.failures !== undefined && performance.totalChecks !== undefined 
    ? Math.max(0, Math.round(((performance.totalChecks - performance.failures) / performance.totalChecks) * 100))
    : null;

  // Get location from settings
  const location = settings?.location || settings?.country || 'Unknown';

  card.innerHTML = `
    <div class="gateway-header">
      <div class="gateway-url">${settings?.fqdn || 'Unknown'}</div>
      <div class="gateway-status ${statusClass}" data-tooltip="${statusText}"></div>
    </div>
    ...
    <div class="gateway-info">
      ...
      <div class="info-row">
        <div class="info-item">
          ...
          <div class="info-value">${uptime !== null ? uptime + '%' : 'N/A'}</div>
        </div>
        <div class="info-item">
...
async function showGatewayDetails(gateway) {
  ...
  // Update subtitle with gateway FQDN
  const subtitle = document.getElementById('modalSubtitle');
  if (subtitle) {
    subtitle.textContent = `${settings.fqdn || 'Unknown Gateway'}`;
  }
